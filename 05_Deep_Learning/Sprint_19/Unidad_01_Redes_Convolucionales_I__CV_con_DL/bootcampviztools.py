...
def pinta_distribucion_categoricas(
    df, columnas_categoricas, relativa=False, mostrar_valores=False
):
...
def plot_categorical_relationship_fin(
    df, cat_col1, cat_col2, relative_freq=False, show_values=False, size_group=5
):
    # Prepara los datos
...
def plot_categorical_numerical_relationship(
    df, categorical_col, numerical_col, show_values=False, measure="mean"
):
        ...
        for i, column in enumerate(columns):
            if df[column].dtype in ["int64", "float64"]:
                ...
                sns.histplot(
                    df[column],
                    kde=True,
                    ax=axes[i, 0] if num_cols > 1 else axes[0],
                ...
                sns.boxplot(
                    x=df[column],
                    ax=axes[i, 1] if num_cols > 1 else axes[1],
                    whis=whisker_width,
                )
                if num_cols > 1:
...
def plot_grouped_histograms(df, cat_col, num_col, group_size):
    ...
    for i in range(0, num_cats, group_size):
        ...
        for cat in subset_cats:
            sns.histplot(
                subset_df[subset_df[cat_col] == cat][num_col], kde=True, label=str(cat)
            )
...
def grafico_dispersion_con_correlacion(
    df, columna_x, columna_y, tamano_puntos=50, mostrar_correlacion=False
):
    """
